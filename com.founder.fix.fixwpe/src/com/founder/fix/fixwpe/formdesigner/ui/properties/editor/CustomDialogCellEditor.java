...
//	private Text displayText;
	...
	public CustomDialogCellEditor(Composite parent, HashMap<String, Object> map) {
		super(parent);
//		this.parent = parent;
//		this.Map = map;

		// 根据配置设置‘只读’属性
//		this.showstate = (String) map.get("showstate"); //$NON-NLS-1$
//		if (this.showstate != null && this.showstate.equals("readonly")) { //$NON-NLS-1$
//			text.setEditable(false);
//		}
	}
	
	public CustomDialogCellEditor(Composite parent, HashMap<String, Object> map, Text displayText) {
		super(parent);
//		this.parent = parent;
		this.Map = map;
		...
		//验证设置不可编辑
		if(map.get("name")!=null&&//$NON-NLS-1$
				map.get("name").toString()!=null//$NON-NLS-1$
				&&map.get("name").toString().contains("verify")){//$NON-NLS-1$ //$NON-NLS-2$
			text.setEditable(false);
		}
...
//		Object showstate = this.Map.get("showstate");//$NON-NLS-1$
//		if (showstate == null) {
//			showstate = "normal";//$NON-NLS-1$
//		}
//
//		Object jsontype = this.Map.get("jsontype");//$NON-NLS-1$
//
//		//选中控件的属性map
//		Map<String, Object> DHtmlPropertyMap = mainEditor.getDhtml()
//				.getElementProperty();

//		Object category = this.Map.get("category");//$NON-NLS-1$
//		String name = this.Map.get("name").toString();//$NON-NLS-1$
		// 事件处理
//		if ("event".equals(category)) {//$NON-NLS-1$
//			this.Map.put(FormConst.FIXCOMPONENTEVENTVALUE, value);
//			mainEditor.setComponentEvent(this.Map);
//		}
//		// 对弹出窗口有设置
//		else if (this.Map.containsKey("dialogfunc")) {//$NON-NLS-1$
...
//				bizobjValue = (String) propertyInfo.get("BizObj");//$NON-NLS-1$
//				fieldsValue = (String) propertyInfo.get("Fields");//$NON-NLS-1$
//				arraylist = _ComponentDialogProperty.getList();
//			}
//			catch(java.lang.ClassCastException e){
//				ChildrenPropertySource _ChildrenPropertySource = (ChildrenPropertySource) this.Map
//						.get(FormConst.COMPONENTDIALOGPROPERTYKEY);
...
//			if(fieldsValue==null
//					&&(Map.get("name").equals("Parent")//$NON-NLS-1$ //$NON-NLS-2$
//					||Map.get("name").equals("Child")//$NON-NLS-1$ //$NON-NLS-2$
//					||Map.get("name").equals("ShowField")//$NON-NLS-1$ //$NON-NLS-2$
//					||Map.get("name").equals("AppendFields"))){//$NON-NLS-1$ //$NON-NLS-2$
//				fieldsValue = value.toString();
//			}
			
//			String serviceValue = (String) propertyInfo.get("Service");//$NON-NLS-1$
//			String dialogfunc = this.Map.get("dialogfunc").toString();//$NON-NLS-1$

//			bizobjId = bizobjValue;

//			if (dialogfunc.startsWith("bizobj:")) {//$NON-NLS-1$
//				if(dialogfunc.equals("bizobj:method")){//$NON-NLS-1$
...
//						// 选择方法的对话框先行
//						if (types.equals("service")) {//$NON-NLS-1$
...
//		}
//		// 组件引用
//		else if (this.Map.containsKey("element"))// (
//		// refc.equals(FormConst.FIXBUTTONCOMPONENTREFC))
//		{
//			String element = this.Map.get("element").toString();
//			// #{func}:advance
//			ElementInfo eleinf = FormPropertyUtils.getElConfExp(element,
//					DHtmlPropertyMap);
//
...
//			}
//
//			Object arrayobject_dialog_state = this.Map
...
//						value.toString(), this.Map);
//				//
//				int rvl = dialog.open();
//
//				// 重新赋值用以下次打开ArrayDialog
//				this.Map.put("arrayobject_dialog_state", "ArrayDialog");
//
//				if (rvl == InputDialog.OK)
...
//				PropertySheetDialog id = new PropertySheetDialog(
...
//			// id,bizobj,
//			// formfilepath
//			String fixcomponents_key = DHtmlPropertyMap.get("fixcomponents_key").toString();
//			String htmlfile = mainEditor.getHtmlFileName();
//
//			
...
//					if(!Map.get("caption").toString().equals("常量")) {
